...
FROM ubuntu:22.04 AS flutterbuilder
RUN apt-get update \
    && apt-get install -y \
        git curl unzip
...
FROM ubuntu:22.04 AS xarbuilder
RUN apt-get update \
	&& apt-get install -y \
...
FROM ubuntu:22.04 AS bomutilsbuilder
RUN apt-get update \
	&& apt-get install -y \
	    git make g++
...
FROM ubuntu:22.04 as appimagebuilder
RUN apt-get update \
	&& apt-get install -y \
	    curl
RUN cd /opt \
...
FROM ubuntu:22.04 AS pacmanbuilder
